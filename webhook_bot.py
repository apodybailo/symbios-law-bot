...
async def latest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.exception("❌ Помилка при обробці пошти")
        await update.message.reply_text(f"❌ Помилка: {str(e)}")

async def echo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info(f"[echo] Повідомлення: {update.message.text} | From ID {update.effective_user.id}")
    if update.effective_user.id != AUTHORIZED_USER_ID:
        return
    await update.message.reply_text("🔍 Для аналізу надішліть /latest або документ.")

if __name__ == "__main__":
    logger.info("🚀 Запуск SYMBIOS AI Юрист через Webhook (PTB 22.0, з логами)")
